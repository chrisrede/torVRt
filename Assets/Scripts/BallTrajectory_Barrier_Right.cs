...
public class BallTrajectory_Barrier_Right : MonoBehaviour {
    ...
    void Start () {
        ...
        startPosBall = new Vector3 (3.72f, 0, -15.34f);
        resetBall ();
    ...
    void shootBall () {
        ball.AddForce (xForce + ((xtarget - 80) / 100), yForce + (ytarget / 100), zForce, ForceMode.Impulse);
    }
    ...
    public void FixedUpdate () {
        ...
        xtarget = rndx.Next (-55, -20);
        ytarget = rndy.Next (-20, 0);
        ...
        if (!isBallLanded) {
            rotatingBall.transform.Rotate (downVecY, -1000 * Time.deltaTime);      
            ...
            ball.AddForce (randomCurve.Next (1, 3) * orthogonalToDirection * distanceFromStartPos*0.8f, ForceMode.Acceleration);
